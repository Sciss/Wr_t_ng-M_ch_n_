...
class MainFrame(c: OSCClient) {
  ...
  private[this] val ggIter1 = Button("Iter1") {
    selection.foreach { instance =>
      c.sendNow(Network.OscIterate(ch = 0, relay = false), instance.socketAddress)
    }
  }

  private[this] val ggIter2 = Button("Iter2") {
    selection.foreach { instance =>
      c.sendNow(Network.OscIterate(ch = 1, relay = false), instance.socketAddress)
    }
  }

  private[this] val ggIter3 = Button("Iter+Relay") {
    selection.foreach { instance =>
      c.sendNow(Network.OscIterate(ch = 0, relay = true), instance.socketAddress)
    }
  }
